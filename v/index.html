...
<head>
    ...
    <title>TalentUp - Strona główna</title>
</head>
...
<body att="to nie dla cb" onload="localStorageLoad(), browserCheck(), setup()">
    ...
    <i id=""></i>
    <i id=""></i>
    shakowałem twój komputer
    <span id="info">test</span>
    <script>
        przegladarka = "niezidentyfikowano";
        const HTrequet = document.getElementById("HTrequest");
        function setup() {
            HTrequet.innerHTML = "Twoja przeglądarka posiada jądro oparte na systemie " + navigator.appName + ". Twój wyświetlacz ma proporcie " + screen.width + " x " + screen.height + ". Twój system to " + navigator.platform;
        }
